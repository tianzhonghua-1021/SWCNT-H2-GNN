...
# 1. path setting
csv_folder = os.path.abspath(os.path.join(os.path.dirname(__file__), '../../../data/gnn/graph_44/data_prepare'))
main_csv = os.path.abspath(os.path.join(os.path.dirname(__file__), '../../../data/gnn/graph_44/dataset_form_noTP.csv'))
dataset_cache_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), '../../../results/graph_44/dmpnn_dataset_cache'))  # cache folder
...
def analyze_dmpnn_uncertainty(train_dataset, test_dataset, transformers, n_iterations=10, save_dir=os.path.abspath(os.path.join(os.path.dirname(__file__), '../../../results/graph_44'))):
    """
...
model = dc.models.torch_models.DMPNNModel(
    ...
    n_tasks=1,
    batch_size=64,
    global_features_size=0,  #no global features
...
SAVE_DIR = os.path.abspath(os.path.join(os.path.dirname(__file__), '../../../results/graph_44'))
...
for i, (x_obj, y_val, w_val, id_val) in enumerate(test_samples):
    ...
    try:
        ...
        print(f"\n Warning: the molecule {fid} failed, error: {e}")
        # skip the failed molecules
        continue

print("\n" + "="*30)
print(f"IG images done")
print(f"Create {success_count}/{total_test} figures successfully")
print(f"The images were saved in: {IG_SAVE_DIR}")
print("="*30)
